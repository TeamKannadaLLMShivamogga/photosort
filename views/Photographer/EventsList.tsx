...
import { 
  ...
  Image as ImageIcon, Filter, LayoutGrid, List, ChevronRight, Mail, Phone, Check, CreditCard, Ticket, Sparkles, ShieldCheck, Eye, Trash2, User, AlertCircle, MapPin, Clock
} from 'lucide-react';
...
const PhotographerEventsList: React.FC<EventsListProps> = ({ onNavigate }) => {
  ...
  // Allow Add-ons to be selected as well
  ...
  // Sync tempSubEvent location with main location if empty
  useEffect(() => {
  ...
  const handleAddSubEvent = () => {
      if (tempSubEvent.name && tempSubEvent.date) {
          const newSub: SubEvent = {
              ...
              location: tempSubEvent.location || newEvent.location // Default to main loc
          };
          setNewEvent({ ...newEvent, subEvents: [...newEvent.subEvents, newSub] });
          // Reset temp but keep location synced
  ...
  const removeSubEvent = (id: string) => {
      setNewEvent({ ...newEvent, subEvents: newEvent.subEvents.filter(se => se.id !== id) });
  };
  ...
  const handleFinalSubmit = async () => {
    ...
    try {
        await addEvent({ 
            ...
            endDate: newEvent.endDate || undefined, // Send undefined if empty string to avoid backend validation error
            ...
            price: Number(newEvent.price) // Ensure number
        });
        resetModal();
    } catch (error) {
                ...
                <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-100">
                      <div className="flex items-center justify-between">
                          ...
                          <span className="text-[9px] text-slate-400">Add days like Haldi, Reception etc.</span>
                      </div>
                      
                      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                              <input type="text" placeholder="Event Name (e.g. Haldi)" className="p-3 bg-white rounded-xl text-xs font-bold outline-none border border-transparent focus:border-[#10B981]" value={tempSubEvent.name} onChange={e => setTempSubEvent({...tempSubEvent, name: e.target.value})} />
                              <input type="date" className="p-3 bg-white rounded-xl text-xs font-bold outline-none border border-transparent focus:border-[#10B981]" value={tempSubEvent.date} onChange={e => setTempSubEvent({...tempSubEvent, date: e.target.value})} />
                              <div className="flex gap-2">
                                  <input type="text" placeholder="Venue" className="flex-1 p-3 bg-white rounded-xl text-xs font-bold outline-none border border-transparent focus:border-[#10B981]" value={tempSubEvent.location} onChange={e => setTempSubEvent({...tempSubEvent, location: e.target.value})} />
                          ...
                          {newEvent.subEvents.map((sub, i) => (
                              <div key={sub.id} className="flex justify-between items-center p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
                                  ...
                                  <button onClick={() => removeSubEvent(sub.id)} className="text-red-400 hover:text-red-600 p-2"><Trash2 className="w-4 h-4" /></button>
                              </div>
                          ))}
                          {newEvent.subEvents.length === 0 && <p className="text-center text-xs text-slate-400 italic py-2">No sub-events added. A default 'Main Event' will be created.</p>}
                      </div>
                  </div>
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-100">
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        ...
                        <input type="email" placeholder="Email (Required)" className="p-3 bg-white rounded-xl text-xs font-bold outline-none" value={tempClient.email} onChange={e => setTempClient({...tempClient, email: e.target.value})} />
                        <div className="flex gap-2">
                      ...
                      {clientList.map((c, i) => (
                          <div key={i} className="flex justify-between items-center p-3 bg-white rounded-xl border border-slate-100 shadow-sm">
                              ...
                              <button onClick={() => removeClient(i)} className="text-red-500"><X className="w-4 h-4" /></button>
                          </div>
                      ))}
                    </div>
                  </div>
